...
import { Term } from "../models/types/Term";
...
export default class MapPNPResult extends ContextService {
    ...
    // This function will map the term field
    private async mapTerm(termField: any): Promise<Term> {
        let result: Term = new Term();
        if (termField) {
            //result = await this.termStoreService.GetTermDetailsByTermKey(termField.TermID);
        }
        return result;
    }
}
